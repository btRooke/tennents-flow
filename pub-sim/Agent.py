...
class Agent:
    def __init__(self, id, venues, start_venue_distribution_path, seed):
        ...
        self.current_venue = self.init_current_venue(venues,
                                                     start_venue_distribution_path)  # Store where the agent is at any time.
        self.coordinates = self.init_coordinates(venues)  # Match the latitude and longitude to the starting location.
        self.preference_distribution = self.init_preference_distribution(
            venues)  # This dictates what the Agent will choose next
    ...
    def __str__(self):
        return f"Agent {self.id}: Budget: {self.budget}, Current Venue: {self.current_venue}, Coordinates: {self.coordinates}, Busy: {self.busy}, Bedtime: {self.bedtime}, Balktime: {self.balktime}, Seed: {self.seed}"
